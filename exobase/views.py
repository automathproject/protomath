...
from exobase.models import Exercice, Folder, OrderFolder, Solution, Profile, Classe, MacroLatex
...
def profile_edit(request):
    ...
    if request.method == "POST":
        ...
        form2 = NewUser2(request.POST, instance = user); test1 = False ; test2 = False
        if form.is_valid():
    ...
    return render(request, 'exobase/profile_edit.html', {'form': form,  'profile': profile, })
...
def auth_edit(request):
    ...
    if request.method == "POST":
        ...
        form = ProfileForm(instance=profile)
        form2 = NewUser2(instance = user)
    return render(request, 'exobase/auth_edit.html', {'form2': form2, 'profile': profile})
...
def solution_edit(request, pk):
    ...
    return render(request, 'exobase/solution_edit.html', {'form': form, 'exercice': exercice, 'solution': solution, 'solution_id': pk, 'reader_list': reader_list, 'classe_list': classe_list,})     
...
def detail_folder(request, folder_id):
    ...
    for e in exercice_in_folder:
        ...
        id_list.append(e.pk)        
    orderfolder_list = OrderFolder.objects.order_by('number').filter(folder = folder)
    number_list = []
    ...
    return render(request, 'exobase/detail_folder.html', {'folder': folder, 'order_ex_list': order_ex_list, 'orderfolder_list': orderfolder_list, 'form': form, 'numbermaxplusun': numbermaxplusun})
...
@permission_required('exobase.delete_folder')
def folder_delete(request,folder_id):
    folder = get_object_or_404(Folder, pk=folder_id)
    user = request.user
    perm = 'exobase.edit_folder_{0}'.format(folder_id)
    if user.has_perm(perm):
        messages.add_message(request, messages.WARNING, u'Le dossier #{0} a bien été supprimé'.format(folder_id))
        folder.delete()
    else:
        messages.add_message(request, messages.WARNING, u'Vous n\'êtes pas autorisé à supprimer ce dossier')
...
@permission_required('exobase.change_folder')
def folder_up_exercice(request,folder_id,exercice_id):
    user = request.user
    perm = 'exobase.edit_folder_{0}'.format(folder_id)
    if not user.has_perm(perm):
        messages.add_message(request, messages.WARNING, u'Vous n\'êtes pas autorisé.e à modifier cet dossier')
    ...
    exercice_in_folder = folder.exercices.all()
    orderfolder_list = OrderFolder.objects.filter(folder = folder)
    if exercice_in_folder.filter(pk = exercice_id).exists():
        exercice = Exercice.objects.get(pk = exercice_id)
        o = OrderFolder.objects.get(exercice=exercice,folder=folder)
        n=o.number
        if n>1:
            o1 = OrderFolder.objects.get(number = n-1, folder = folder)
            o.number=n-1
            o.save()
            o1.number=n
            o1.save()
        ...
        messages.add_message(request, messages.WARNING, u'L\'exercice {0} n\'existe pas.'.format(str(ex_id)))
    return redirect('folder_resume',folder_id=folder_id)

@permission_required('exobase.change_folder')
def folder_down_exercice(request,folder_id,exercice_id):
    user = request.user
    perm = 'exobase.edit_folder_{0}'.format(folder_id)
    if not user.has_perm(perm):
        messages.add_message(request, messages.WARNING, u'Vous n\'êtes pas autorisé.e à modifier cet dossier')
    ...
    exercice_in_folder = folder.exercices.all()
    orderfolder_list = OrderFolder.objects.filter(folder = folder)
    number_list=orderfolder_list.values_list('number',flat=True)
        ...
        if form:
            ...
            if username == authorname:
                ...
                if User.objects.filter(username = username).exists():
                    user = get_object_or_404(User,username=username)
                    perm_name = 'see_exercice_{0}'.format(exercice_id)
                    perm_name_app = 'exobase.see_exercice_{0}'.format(exercice_id)
                    perm = Permission.objects.get(codename=perm_name)
                    if user.has_perm(perm_name_app):
                        user.user_permissions.remove(perm)
                else:
                    messages.add_message(request, messages.WARNING, u'Cet utilisateur n\'existe pas')
        else:
